    ...
    public function authenticated()
{
    ...
    if ($user->user_type === 'admin' || $user->user_type === 'staff') {
        ...
        if (empty($user->dashboard_token)) {
            $user->dashboard_token = \Illuminate\Support\Str::random(30);
            $user->save();
        }

        // ✅ Redirect to tokenized dashboard
        return redirect()->route('seller.dashboard.token', ['token' => $user->dashboard_token]);
    }

    // ✅ For other users (e.g., customers)
    if (session('link') != null) {
        return redirect(session('link'));
    } else {
        return redirect()->route('dashboard');
