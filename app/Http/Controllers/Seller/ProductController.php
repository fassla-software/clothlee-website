...
class ProductController extends Controller
{
  ...
  public function fabrics(Request $request) {
    // Fetch product counts per fabric
    $productCounts = Product::whereNotNull('fabric')
        ->where('fabric', '!=', '')
        ->where('user_id', Auth::user()->id)
        ->select('fabric', DB::raw('COUNT(DISTINCT id) as product_count'))
        ->groupBy('fabric')
        ->get()
        ->keyBy('fabric'); // Convert to collection indexed by fabric

    // Fetch order counts per fabric using OrderDetails
    $orderCounts = OrderDetail::whereNotNull('fabric')
        ->where('fabric', '!=', '')
      	->where('seller_id', Auth::user()->id)
        ->select('fabric', DB::raw('COUNT(order_id) as order_count'))
        ->groupBy('fabric')
        ->get()
        ->keyBy('fabric'); // Convert to collection indexed by fabric

    // Merge results into a collection of objects
    $fabrics = collect();
    foreach ($productCounts as $fabric => $productData) {
        $fabrics->push((object) [
            'fabric' => $fabric,
            'product_count' => $productData->product_count,
            'order_count' => $orderCounts[$fabric]->order_count ?? 0, // Default to 0 if no orders exist
        ]);
    }

    // Apply search filter
    if ($request->has('search') && !empty($request->search)) {
        $fabrics = $fabrics->filter(function ($item) use ($request) {
            return stripos($item->fabric, $request->search) !== false;
        });
    }
    ...
    $fabrics = new \Illuminate\Pagination\LengthAwarePaginator(
        $fabrics->forPage($currentPage, $perPage),
        $fabrics->count(),
        $perPage,
        $currentPage,
    ...
    return view('seller.product.fabric.index', compact('fabrics'));
}
...
public function updatePrice(Request $request, $id)
{
    ...
    foreach ($variants as $variant) {
        $variant->price = $request->unit_price;  // Update the price for each variant
...
public function showUpdatePriceForm($productId)
{
    // Get product and its variants
    $product = Product::findOrFail($productId);
    $variants = ProductStock::where('product_id', $productId)->get();

    return view('update-price', compact('product', 'variants'));
}

public function updateVariantPrice(Request $request, $productId)
{
    // Validate the input
    $request->validate([
    ...
    return redirect()->route('product.update-price', $productId)->with('success', 'Price updated successfully.');
}
...
public function index(Request $request)
{
    ...
    // Search by product name if search parameter is provided
    ...
    // Attach both the selected child category and its parent
    $categoryIds = [$request->category_id, $request->parent_category];
