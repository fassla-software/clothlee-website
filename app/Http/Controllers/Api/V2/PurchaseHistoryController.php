    ...
    public function items($id)
{
    ...
    $order_query = OrderDetail::where('order_id', $order_id->id)->with('product.categories')->get();

    // Transform the response to include the subcategory name
    $order_query->transform(function ($orderDetail) {
        $product = $orderDetail->product;

        if ($product) {
            // Fetch the first subcategory (level 1)
            $subcategory = $product->categories->where('level', 1)->first();
            $subcategory_name = $subcategory ? $subcategory->getTranslation('name') : translate('No Subcategory');
        } else {
            $subcategory_name = translate('No Subcategory');
        }

        // Modify the response with subcategory_name
        return [
            "id" => $orderDetail->id,
            "product_id" => $orderDetail->product_id,
            "product_name" => $product ? $product->getTranslation('name') : translate('Unknown Product'),
            "subcategory_name" => $subcategory_name,
            "variation" => $orderDetail->variation,
            "price" => single_price($orderDetail->price),
            "tax" => single_price($orderDetail->tax),
            "shipping_cost" => single_price($orderDetail->shipping_cost),
            "coupon_discount" => single_price($orderDetail->coupon_discount),
            "quantity" => $orderDetail->quantity,
            "payment_status" => $orderDetail->order->payment_status,
            "payment_status_string" => ucfirst($orderDetail->order->payment_status),
            "delivery_status" => $orderDetail->delivery_status,
            "delivery_status_string" => ucfirst($orderDetail->delivery_status),  // ✅ FIXED THIS LINE
   ...
   public function re_order($id)
{
    ...
    foreach ($order->orderDetails as $key => $orderDetail) {
        ...
        // Fetch the subcategory (level 1 category)
        $subcategory = $product->categories()->where('level', 1)->first();
        $subcategory_name = $subcategory ? $subcategory->getTranslation('name') : translate('No Subcategory');
        ...
        if ($product_stock) {
            ...
            if ($product->digital != 1) {
                ...
                if ($quantity > 0) {
                    ...
                    // If order qty is greater than the product stock, set order qty = current product stock qty
                    $quantity = ($quantity >= $order_qty) ? $order_qty : $quantity;
                } else {
                    array_push($failed_msgs, $subcategory_name . ' ' . translate('is stock out.'));
                    continue;
                }
            }
            ...
            array_push($success_msgs, $subcategory_name . ' ' . translate('added to cart.'));
        } else {
            array_push($failed_msgs, $subcategory_name . ' ' . translate('is stock out.'));
        }
    }
