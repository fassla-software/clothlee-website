...
{
    \Log::info('📲 update_device_token called', $request->all());

    $user = null;

    // ✅ First priority: if dashboard_token is sent, use it
    if ($request->filled('dashboard_token')) {
        \Log::info('🔍 Searching by dashboard_token: ' . $request->dashboard_token);
        $user = \App\Models\User::where('dashboard_token', $request->dashboard_token)->first();
        if ($user) {
            \Log::info('👤 Found user by dashboard_token: ' . $user->id);
        }
    }

    // ❌ Only fallback to auth user if dashboard_token is missing
    if (!$user && auth()->check()) {
        $user = auth()->user();
        \Log::info('✅ Fallback to Authenticated user: ' . $user->id);
    }

    if (!$user) {
        \Log::warning('❌ No user found for device token update');
        return response()->json([
            'result' => false,
    ...
    \Log::info('🧾 Fresh DB value:', ['device_token' => $user->fresh()->device_token]);
