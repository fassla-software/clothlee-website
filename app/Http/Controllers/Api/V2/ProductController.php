...
class ProductController extends Controller
{
    ...
    public function categoryProducts($slug, Request $request)
    {
      ...
      // 🔹 Extract numeric values from price_min and price_max
    $min_price = $request->has('price_min') ? (float) preg_replace('/[^0-9.]/', '', $request->price_min) : 0;
    $max_price = $request->has('price_max') ? (float) preg_replace('/[^0-9.]/', '', $request->price_max) : PHP_INT_MAX;

      
      // 🔹 Correctly Calculate Discounted Price Before Filtering
    $products->whereRaw("
        (unit_price - CASE 
            WHEN discount_type = 'percent' THEN (unit_price * discount / 100) 
            WHEN discount_type = 'amount' THEN discount 
            ELSE 0 
        END) BETWEEN ? AND ?", [$min_price, $max_price]);
    ...
    public function search(Request $request)
    {
        ...
        $brand_ids = [];
$sub_category_ids = [];
if ($request->has('sub_categories') && !empty($request->sub_categories)) {
    $sub_category_ids = explode(',', $request->sub_categories);
}
       ...
       if (!empty($category_ids) || !empty($sub_category_ids)) {
    $all_category_ids = array_merge($category_ids, $sub_category_ids);
    $n_cid = [];
    foreach ($all_category_ids as $cid) {
        $n_cid = array_merge($n_cid, CategoryUtility::children_ids($cid));
    }

    if (!empty($n_cid)) {
        $all_category_ids = array_merge($all_category_ids, $n_cid);
    }

    $products->whereIn('category_id', $all_category_ids);
}
