...
$customers = User::whereIn('user_type', ['customer', 'seller', 'admin', 'staff']) // include types you want
                 ->where('banned', 0)
                 ->get();
    ...
    public function sendCustomNotification(Request $request)
{
    ...
    $title = $notificationType?->getTranslation('title') ?? 'Notification';
    $body  = $notificationType?->getTranslation('default_text') ?? '';
...
foreach ($request->user_ids as $user_id) {
    ...
    if (!empty($user->device_token)) {
        ...
        if ($user->user_type === 'seller') {
            $link = 'https://www.clothlee.com/seller/dashboard/';
        }
        ...
        $fields = [
            'message' => [
                ...
                'data' => [
                    ...
                    'type' => $user->user_type === 'seller' ? 'webview' : 'default',
                    'link' => $link,
                ]
            ]
        ];
