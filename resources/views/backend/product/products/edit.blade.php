...
<div class="card-body">
    <ul class="list-unstyled">
        @foreach ($categories as $category)
            <li>
                <h6>{{ $category->name }}</h5> <!-- Main category without radio -->
...
<script type="text/javascript">
  $(document).ready(function() {
    ...
    const minQtyHint = document.getElementById('min_qty_hint');

    // Set initial state based on the product's stored value
    function initializeState() {
        if (toggle.checked) {
            toggleLabel.textContent = 'Wholesale';
            minQtyWrapper.style.display = 'block';
            minQtyInput.min = 2;
            minQtyHint.style.display = 'block';
        } else {
            toggleLabel.textContent = 'Retail';
            minQtyWrapper.style.display = 'none';
            minQtyInput.min = 1;
            minQtyHint.style.display = 'none';
        }
    }
    ...
    toggle.addEventListener('change', function () {
        if (toggle.checked) {
            toggleLabel.textContent = 'Wholesale';
            minQtyWrapper.style.display = 'block';
            minQtyInput.value = Math.max(minQtyInput.value, 2); // Ensure it's at least 2
            minQtyInput.min = 2;
            minQtyHint.style.display = 'block';
        } else {
            toggleLabel.textContent = 'Retail';
            minQtyWrapper.style.display = 'none';
            minQtyInput.value = 1; // Reset to 1 in Retail mode
            minQtyInput.min = 1;
            minQtyHint.style.display = 'none';
        }
    });

    initializeState(); // Set correct state on page load
});
